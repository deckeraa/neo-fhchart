...
class DefaultPlotEditor extends JPanel implements ActionListener {
	 ...
	 */
	private FHAESDefaultAxisEditor domainAxisPropertyPanel;

	/**
	 * A panel used to display/edit the properties of the range axis (if any).
	 */
	private FHAESDefaultAxisEditor rangeAxisPropertyPanel;
	...
	protected static ResourceBundle localizationResources = ResourceBundleWrapper
			.getBundle("gov.frames.fhaes.jfreechart.editor.LocalizationBundle");

	/**
	 * Standard constructor - constructs a panel for editing the properties of the specified plot.
	 * <P>
	 * In designing the panel, we need to be aware that subclasses of Plot will need to implement subclasses of PlotPropertyEditPanel - so
	 * we need to leave one or two 'slots' where the subclasses can extend the user interface.
	 * 
...
// interior.add(button);
		...
		this.domainAxisPropertyPanel = FHAESDefaultAxisEditor.getInstance(domainAxis);
		...
		this.rangeAxisPropertyPanel = FHAESDefaultAxisEditor.getInstance(rangeAxis);
...
// dmo: (end dmo additions)
		...
		JPanel panelGrids = new JPanel();
		// tabs.addTab("Grid", null, panelGrids, null);
		panelGrids.setLayout(new MigLayout("", "[][]", "[]"));

		JLabel lblShowhideGrid = new JLabel("Show/hide grid");
		panelGrids.add(lblShowhideGrid, "cell 0 0");

		JCheckBox chkGrid = new JCheckBox("");
		panelGrids.add(chkGrid, "cell 1 0");
	 ...
	 */
	public FHAESDefaultAxisEditor getDomainAxisPropertyEditPanel() {
	 ...
	 * @return A reference to a panel.
	 */
	public FHAESDefaultAxisEditor getRangeAxisPropertyEditPanel() {
...
// }
	...
	/**
	 * Allow the user to change the outline paint. We use JColorChooser, so the user can only choose colors (a subset of all possible
	 * paints).
	 */
...
//
	...
	/**
	 * Allow the user to modify whether or not lines are drawn between data points by <tt>LineAndShapeRenderer</tt>s and
	...
	/**
	 * Allow the user to modify whether or not shapes are drawn at data points by <tt>LineAndShapeRenderer</tt>s and
	 * <tt>StandardXYItemRenderer</tt>s.
	 */
